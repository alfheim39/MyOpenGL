...
namespace Mesh {
	...
	*/
	...
	/**
	* @desc FBX オブジェクトを破棄するためのヘルパー構造体
	*/
	template<typename T>
	struct Deleter {
	...
	*/
	bool FbxLoader::LoadMesh(FbxNode* fbxNode) {
		...
		//頂点がどのマテリアルに属するかあを示すマテリアルリストを取得する
	...
	*/
	...
	/**
	* @desc メッシュを描画する
	* @param buffer	描画に使用するメッシュバッファーへのポインタ
	*/
	...
	*/
	...
	/**
	* @desc バッファが保持するVAOをOpenGLの処理対象に設定する
	*/

...
namespace Shader {
	/**
	* @desc シェーダープログラムを作成する
	* @param vsCode	頂点シェーダーコードへのポインタ.
	* @param fsCode	フラグメントシェーダーコードへのポインタ
	...
	*/
	ProgramPtr Program::Create(const char* vsFilename, const char* fsFilename) {
		...
		//頂点シェーダーファイル名の末尾から.vertを取り除いたものをプログラム名とする
		p->name = vsFilename;
	...
	*/
	...
	/**
	* @desc Uniformブロックをバインディング・ポイントに割り当てる
	* @param blockName		割り当てるUniformブロックの名前
	* @param bindingPoint	割り当てる先のバインディング・ポイント
	* @retval	割当成功
	* @retval	割当失敗
	*/
	bool Program::UniformBlockBinding(const char* blockName, GLuint bindingPoint) {
		...
		if (blockIndex == GL_INVALID_INDEX) {
			std::cerr << "1ERROR(" << name << "): Uniformブロック`" << blockName << "のバインドに失敗 " << std::endl;
			return false;
		}
		...
		if (result != GL_NO_ERROR) {
			std::cerr << "2ERROR(" << name << "):Uniformブロック" << blockIndex << "のバインドに失敗" << std::endl;
			return false;
		}
		return true;
	}
	...
	*/
	...
	/**
	* @desc	テクスチャをテクスチャ・イメージ・ユニットに割り当てる
